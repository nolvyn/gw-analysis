...
def init_plot():
    plt.figure(figsize=(10, 5), dpi=constants.DPI)
    plt.grid(True)
...
def get_parameters(event):
    ...
    data = h5py.File(fn, 'r')
    return data[aprx_list[0]]['posterior_samples'][:constants.DRAW_QUANTITY]
...
def calculate_amp(h1, h2):
    return (np.abs(h1) - np.abs(h2)) / (np.abs(h1) + constants.SMALL)

def calculate_phase(h1, h2):
    return np.unwrap(np.angle(h1)) - np.unwrap(np.angle(h2))

def calculate_r_phase(d_phi, h1):
    psd_obj = calculate_psd(len(h1), constants.DELTA_F, constants.F_LOWER)
    ref_amplitude = np.abs(h1)
    ref_sigma = np.interp(h1.sample_frequencies, h1.sample_frequencies, psd_obj) + constants.SMALL
    align_weights = (ref_amplitude ** 2) / ref_sigma
    fit = np.polynomial.Polynomial.fit(
        h1.sample_frequencies, d_phi, 1, w=align_weights,
        domain=[constants.F_LOWER, h1.sample_frequencies[-1]]
    )
    return d_phi - fit(h1.sample_frequencies)

def calculate_mismatch(h1, h2):
    psd_obj = calculate_psd(len(h1), constants.DELTA_F, constants.F_LOWER)
    match_val, _ = match(h1, h2, psd=psd_obj, low_frequency_cutoff=constants.F_LOWER)
    return 1 - match_val
...
def truncate_waveform(waveforms, wfm_a, wfm_b):
    ...
    threshold_amp = constants.THRESH_NUM * amp_ref[idx_max_amp]
